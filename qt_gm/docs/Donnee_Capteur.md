...
| :--- | :--- | :--- | :--- | :--- |
| **État global salle** | VL53L0X + HC-SR04 + Config capacité | Occupation + anticipation | `salle/{id}/etat`<br>`{"occupation": 24, "capacite": 30, "densite": 0.80, "anticipation_min": 8, "en_ligne": true}` | Dashboard Qt Zone 2+3, Asterisk |
...
| Reset alerte | `salle/{id}/alerte/reset` | `{"type": "all"}` | Acquittement local, LED retour normal |
...
5.  **Topics structurés hiérarchiquement** : `salle/{id}/categorie/signal`. Permet subscription wildcard (`salle/+/etat`) pour supervision globale et subscription précise (`salle/B204/alerte/intrusion`) pour traitement spécifique. Scalable nativement.
